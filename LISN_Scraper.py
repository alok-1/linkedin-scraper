from helpers import get_inputs, make_browser, print_app_end, quit_browser, get_defaults, get_links, get_data, make_json_to_csv, make_json, get_inputs, print_app_info, get_filters, get_inputs_page_no
...
class App:
    def __init__(self) -> None:
        ...
        self.scrape_data_start_page_no: int = get_inputs_page_no(self.browser)

        self.filters: dict = get_filters(self.browser)

        self.result_links: list = get_links(
            self.browser, self.scrape_data_amount, self.scrape_data_start_page_no)
